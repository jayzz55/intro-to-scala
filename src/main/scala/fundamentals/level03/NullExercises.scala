...
object NullExercises {
  ...
  def mkPersonOrNull(name: String, age: Int): Person =
    (name, age) match {
      case (name, _) if name.isEmpty => null 
  ...
  def mkPersonOrNullThenChangeName(oldName: String, age: Int, newName: String): Person =
    (mkPersonOrNull(oldName, age), newName) match {
      case (person: Person, newName: String) if !newName.isEmpty => changeName(newName, person)
      case _ => null
    }
