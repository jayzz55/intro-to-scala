...
object TryExercises {
  ...
  def mkEmployee(csv: String): Either[TryError, Employee] =
    csv.split(",") match {
      case Array(nameStr, ageStr, hasDirectReportsStr) => parseEmployee(nameStr, ageStr, hasDirectReportsStr)
      case _ => Left(TryError("CSV has wrong number of fields. Expected 3."))
    }

  def parseEmployee(nameStr: String, ageStr: String, hasDirectReportsStr: String) =
    for {
      ageInt <- tryToEither(parseIntSafe(ageStr))
      hasDirectReportsBoolean <- tryToEither(parseBooleanSafe(hasDirectReportsStr))
    } yield Employee(nameStr, ageInt, hasDirectReportsBoolean)
