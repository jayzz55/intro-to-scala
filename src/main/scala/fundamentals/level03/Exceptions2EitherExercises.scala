...
object Exceptions2EitherExercises {
  ...
  def getAge(providedAge: String): Either[AppError, Int] =
    try {
      providedAge.toInt match {
        case x if 1 until 121 contains x => Right(x)
        case _ => Left(InvalidAgeRange(s"provided age should be between 1-120: $providedAge"))
      }
    } catch {
  ...
  def createPerson(name: String, age: String): Either[AppError, Person] =
    for {
      ...
      ageValue <- getAge(age)
    } yield Person(nameValue, ageValue)
  ...
  def makeNameUpperCase(name: String, age: String): Either[AppError, Person] = 
    createPerson(name, age).map(person => person.copy(name = name.toUpperCase))

  /**
    * Reimplement createPerson using only `flatMap` and `map`
    */
 def createPerson2(name: String, age: String): Either[AppError, Person] =
   getName(name).flatMap(nameValue => getAge(age).map(ageValue => Person(nameValue, ageValue)))
