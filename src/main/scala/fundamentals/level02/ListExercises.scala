...
object ListExercises {
  ...
  def showListSize[A](xs: List[A]): String =
    xs match {
      ...
      case xs:List[A] => s"This is a list of size ${xs.length}"
    }
  ...
  def min(nums: List[Int]): Int =
    nums match {
      ...
      case nums:List[Int] => nums.foldLeft(Int.MaxValue) { (acc, i) => if (i < acc) i else acc }
    }
  ...
  def youngestPerson(persons: List[Person]): Person =
    persons match {
      ...
      case persons: List[Person] => persons.foldLeft(persons.head) { (acc,i) => if (i.age < acc.age) i else acc }
    }
  ...
  def showEveryNthPerson(n: Int, persons: List[Person]): List[String] =
    persons.zipWithIndex.filter{
      case (v, i) => (i+1) % n == 0
    }.map( tuple => showPerson1(tuple._1) )
}
